...
up-gunicorn:
	...
	# @echo ".venv/bin/ddtrace-run .venv/bin/gunicorn server_all:app --reload --name nabla-site-python --workers 1 -k uvicorn_worker.UvicornWorker --bind 0.0.0.0:$(PORT) --logger-class=nabla.utils.log_config.JMGunicornLogger --log-level info --access-logfile - --statsd-host localhost:8125"
	.venv/bin/ddtrace-run .venv/bin/gunicorn server_all:app --reload --name nabla-site-python -k uvicorn_worker.UvicornWorker \
	--workers $(NPROC) \
