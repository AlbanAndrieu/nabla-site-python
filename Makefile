...
up-uvicorn:
	...
	@echo ".venv/bin/uvicorn api.main:app --reload --workers 1 --host 0.0.0.0 --port $(PORT)"
	.venv/bin/uvicorn api.main:app --reload --workers $(NPROC) --host 0.0.0.0 --port $(PORT) \
...
up-gunicorn:
	...
	# @echo ".venv/bin/ddtrace-run .venv/bin/gunicorn api.main:app --reload --name nabla-site-python --workers 1 -k uvicorn_worker.UvicornWorker --bind 0.0.0.0:$(PORT) --logger-class=nabla.utils.log_config.JMGunicornLogger --log-level info --access-logfile - --statsd-host localhost:8125"
	.venv/bin/ddtrace-run .venv/bin/gunicorn api.main:app --reload --name nabla-site-python -k uvicorn_worker.UvicornWorker \
	--workers $(NPROC) \
